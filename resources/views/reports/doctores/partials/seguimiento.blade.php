    ...
    <script>
        (() => {
            ...
            const initSeguimientoModule = () => {
                ...
                const ensureDoctorNames = processedData => {
                    ...
                    const applyResolvedName = doctor => {
                        ...
                        const preferredName = doctor.name || doctor.doctor || doctor.doctor_name ||
                            doctorNameCache.get(doctor.id_doctor) || getFallbackDoctorName(doctor
                            .id_doctor);
                    ...
                    const chart = new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                    ...
                                    data: dataset.map(item => Number(item[config.positiveKey]) ||
                                        0),
                                ...
                                },
                                {
                                    ...
                                    data: dataset.map(item => Number(item[config.negativeKey]) ||
                                        0),
                ...
                // Orden actual de la tabla: 'negatives' para mostrar primero negativos, 'positives' para mostrar positivos
                let currentOrdering = 'negatives';

                const orderButton = document.getElementById('order_table');

                const formatOrderButtonLabel = ordering => {
                    // Indica la acción que ocurrirá al presionar el botón (flip)
                    return ordering === 'negatives' ? 'Negativos' : 'Positivos';
                };

                const setOrderButtonState = ordering => {
                    if (!orderButton) return;
                    orderButton.textContent = formatOrderButtonLabel(ordering);
                    orderButton.dataset.ordering = ordering;
                };
                ...
                const sortDoctorComparisons = (doctors = [], ordering = 'negatives', metric = 'amount') => {
                    ...
                    return [...doctors].sort((a, b) => {
                        ...
                        if (ordering === 'negatives') {
                            // Priorizar valores negativos (más negativos al principio). Cuando ambos positivos, mantener menor primero
                            // Para mostrar el "top" negativo, ordenamos ascendente por valor (más negativos -> más pequeños).
                            return valA - valB;
                        }

                        // Positives: queremos mostrar valores positivos más grandes primero
                        return valB - valA;
                    });
                };

                // Toggle que alterna entre Negativos y Positivos
                if (orderButton) {
                    orderButton.addEventListener('click', () => {
                        ...
                        // Reordenar la tabla usando la comparativa procesada
                ...
                const normalizeDoctorEntry = (entry = {}) => {
                    ...
                    const name = entry.doctor_name || entry.doctor || entry.name || getFallbackDoctorName(
                        idDoctor);
                    ...
                    const amountDiff = Number(entry.amount_fluctuation ?? entry.amount_diff ?? (currAmount -
                        prevAmount));
                    const quantityDiff = Number(entry.quantity_fluctuation ?? entry.quantity_diff ?? (
                ...
                const processBackendData = payload => {
                    ...
                    const fallbackPositiveAmount = buildSeriesFromComparisons(doctorComparisons, 'amount_diff',
                        'positive');
                    const fallbackNegativeAmount = buildSeriesFromComparisons(doctorComparisons, 'amount_diff',
                        'negative');
                    const fallbackPositiveQuantity = buildSeriesFromComparisons(doctorComparisons,
                        'quantity_diff', 'positive');
                    const fallbackNegativeQuantity = buildSeriesFromComparisons(doctorComparisons,
                        'quantity_diff', 'negative');
                    ...
                    const comparisonsForTrend = doctorComparisons.length ?
                        doctorComparisons :
                        [...positiveByAmount, ...negativeByAmount, ...positiveByQuantity, ...
                        negativeByQuantity];
                ...
                const updateStatistics = stats => {
                    ...
                    if (negativeQuantityEl) {
                        negativeQuantityEl.textContent =
                            `${formatSignedQuantity(stats.avgNegativeQuantity)} pedidos`;
                    }
                    ...
                    if (positiveQuantityEl) {
                        positiveQuantityEl.textContent =
                            `${formatSignedQuantity(stats.avgPositiveQuantity)} pedidos`;
                    }
                    ...
                    if (trendSummaryEl) {
                        const amountLabel =
                            `${currencyFormatter.format(stats.firstAverageAmount)} → ${currencyFormatter.format(stats.secondAverageAmount)}`;
                        const quantityLabel =
                            `${quantityFormatter.format(stats.firstAverageQuantity)} → ${quantityFormatter.format(stats.secondAverageQuantity)}`;
                        trendSummaryEl.textContent =
                            `Promedio monto: ${amountLabel} | Promedio pedidos: ${quantityLabel}`;
                    }
                };
                ...
                const updateDoctorsTable = doctors => {
                    ...
                    if (!doctors.length) {
                        tbody.innerHTML =
                            '<tr><td colspan="8" class="text-center text-muted py-4">No hay información disponible para los filtros seleccionados.</td></tr>';
                        if (summary) {
                    ...
                    tbody.innerHTML = doctors.map((doctor, index) => {
                        const amountDiff = Number(doctor.amount_diff) || 0;
                        const quantityDiff = Number(doctor.quantity_diff) || 0;
                        const amountTrendClass = amountDiff >= 0 ? 'badge-grobdi badge-green' :
                            'badge-grobdi badge-red';
                        const quantityTrendClass = quantityDiff >= 0 ? 'badge-grobdi badge-green' :
                            'badge-grobdi badge-red';
                    ...
                    if (summary) {
                        ...
                        summary.textContent =
                            `Mostrando ${doctors.length} doctores (${positives} positivos / ${negatives} negativos)`;
                    }
                ...
                const renderReport = async (filters = {}) => {
                    ...
                    appState.lastRequestFilters = {
                        ...requestFilters
                    };

                    if (!appState.initialFilters) {
                        appState.initialFilters = {
                            ...appState.filters
                        };
                        appState.initialRequestFilters = {
                            ...requestFilters
                        };
                    }
                    ...
                    // Actualizar tabla (ordenado segun la preferencia actual del usuario)
                    ...
                    if (appState.charts.positive) {
                        updateHorizontalChart(appState.charts.positive, datasets.positive, currentMetric,
                            'positives');
                    } else {
                        appState.charts.positive = buildHorizontalChart('chartSeguimientoMax', datasets
                            .positive, currentMetric, 'positives');
                    }

                    if (appState.charts.negative) {
                        updateHorizontalChart(appState.charts.negative, datasets.negative, currentMetric,
                            'negatives');
                    } else {
                        appState.charts.negative = buildHorizontalChart('chartSeguimientoMin', datasets
                            .negative, currentMetric, 'negatives');
                    }

                    if (appState.charts.trend) {
                        updateMonthlyTrendChart(appState.charts.trend, processedData.monthlyPerformance,
                            currentMetric);
                    } else {
                        appState.charts.trend = buildMonthlyTrendChart('chartSeguimientoTrend',
                            processedData.monthlyPerformance, currentMetric);
                    }
                };
                const handleMetricChange = metric => {
                    ...
                    // Reordenar la tabla actual según la preferencia de orden y la nueva métrica
                    if (appState.data) {
                ...
                resetButton?.addEventListener('click', () => {
                    ...
                    if (Object.keys(baselineFilters).length) {
                        ...
                        ['range_a_start', 'range_a_end', 'range_b_start', 'range_b_end'].forEach(key => {
                            ...
                            if (input) {
                                const initial = input.dataset.initialDefault || input.dataset
                                    .default || '';
                                input.value = initial;
                            }
                        });
                    ...
                    // Restablecer orden de tabla a predefinido
