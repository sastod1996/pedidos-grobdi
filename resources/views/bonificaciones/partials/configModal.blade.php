<div class="modal fade modal-grobdi" id="configuracionModal" tabindex="-1" role="dialog"
    aria-labelledby="configuracionModalLabel" aria-hidden="true">
                    ...
                    <div class="tab-pane fade" id="form-pane" role="tabpanel" aria-labelledby="form-tab">
                        <form id="configuracionRangoForm"
                                    ...
                                    <div class="row g-3 align-items-end">
                                        <div class="col-12 col-md-4">
                                            <div class="form-group">
                                                <label class="form-label">Porcentaje inicial</label>
                                                <input type="number" class="form-control range-input" min="0"
                                                    max="99.99" step="0.01" data-field="inicio"
                                                    placeholder="Ej. 0" value="0">
                                            </div>
                                        </div>
                                        <div class="col-12 col-md-4">
                                            <div class="form-group">
                                                <label class="form-label">Porcentaje final</label>
                                                <input type="number" class="form-control range-input" min="0"
                                                    max="99.99" step="0.01" data-field="fin" placeholder="Ej. 79"
                                                    value="79">
                                            </div>
                                        </div>
                                        <div class="col-12 col-md-3">
                                            <div class="form-group">
                                                ...
                                                <input type="number" class="form-control range-input" min="0"
                                                    max="99.99" step="0.01" data-field="comision"
                                                    placeholder="Ej. 0" value="0">
                                            </div>
                                        </div>
                            ...
                            <div class="mt-3 d-flex gap-2">
                                <button type="button" class="btn btn-outline" id="addRangeRow">
                                    <i class="fas fa-plus"></i> Agregar otro rango
                                </button>
                                <button type="submit" class="btn btn-primary" id="saveRangeConfig">Guardar
                                    rangos</button>
                            </div>
                        </form>
                    </div>
                            ...
                            <table class="table table-striped mb-0 align-middle table-grobdi">
                                ...
                                <tbody id="configTableBody">
                                    <tr class="text-center">
                                        <td colspan="3">No hay configuración activa.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
...
<script>
    ...
    window.addEventListener('load', function() {
        ...
        $(document).on('click', 'button[data-toggle="tab"], button[data-bs-toggle="tab"]', function(e) {
            ...
            const $tabContent = $($btn.closest('[role="tablist"]').attr('data-target') ||
                '#configTabsContent');
        ...
        $(document).on('hidden.bs.modal', '#configuracionModal', function() {
            const $firstBtn = $(this).find('button[data-toggle="tab"], button[data-bs-toggle="tab"]')
                .first();
        ...
        (function() {
            ...
            function makeRow(index, values) {
                ...
                var $row = $(
                    ...
                    '      <input type="number" class="form-control range-input" min="0" max="99.99" step="0.01" data-field="inicio" placeholder="Ej. 0" value="' +
                    values.inicio + '">' +
                    '    </div>' +
                    '    <div class="col-12 col-md-4">' +
                    '      <label class="form-label small text-uppercase text-muted">Porcentaje final</label>' +
                    '      <input type="number" class="form-control range-input" min="0" max="99.99" step="0.01" data-field="fin" placeholder="Ej. 79" value="' +
                    values.fin + '">' +
                    '    </div>' +
                    '    <div class="col-12 col-md-3">' +
                    '      <label class="form-label small text-uppercase text-muted">Comisión (%)</label>' +
                    '      <input type="number" class="form-control range-input" min="0" max="99.99" step="0.01" data-field="comision" placeholder="Ej. 0" value="' +
                    values.comision + '">' +
                    '    </div>' +
                    '    <div class="col-12 col-md-1 text-md-right">' +
            ...
            $modal.on('click', '#addRangeRow', function(e) {
                ...
                $rangeContainer.find('.remove-range-row').toggle($rangeContainer.find(
                    '.bonificaciones-range-group').length > 1);
            });
            ...
            function validateRanges(ranges) {
                ...
                ranges.forEach(function(r, idx) {
                    ...
                    if (r.inicio < 0 || r.inicio > 99.99) msgs.push(
                        'Porcentaje inicial fuera de rango en fila ' + (idx + 1));
                    if (r.fin < 0 || r.fin > 99.99) msgs.push(
                        'Porcentaje final fuera de rango en fila ' + (idx + 1));
                    if (r.com < 0 || r.com > 99.99) msgs.push('Comisión fuera de rango en fila ' + (
                        idx + 1));
                    if (r.inicio > r.fin) msgs.push(
                        'El porcentaje inicial debe ser menor o igual al porcentaje final en fila ' +
                        (idx + 1));
                });
                ...
                for (var i = 0; i < sorted.length - 1; i++) {
                    ...
                    if (cur.fin >= next.inicio) {
                        msgs.push('Los intervalos se solapan: [' + cur.inicio + ',' + cur.fin + '] y [' +
                            next.inicio + ',' + next.fin + ']');
                        break;
                    }
                }
                return msgs;
            ...
            $modal.on('submit', '#configuracionRangoForm', function(e) {
                ...
                if (errors.length) {
                    if (window.Swal) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Errores',
                            html: '<ul style="text-align:left">' + errors.map(function(m) {
                                return '<li>' + m + '</li>';
                            }).join('') + '</ul>'
                        });
                    } else {
                ...
                var details = ranges.map(function(r) {
                    return {
                        initial_percentage: r.inicio,
                        final_percentage: r.fin,
                        commission: r.com
                    };
                });
                var payload = {
                ...
                fetch(action, {
                    ...
                    if (result.ok && result.data && result.data.success) {
                        ...
                        // close modal
                        try {
                            var modalEl = document.getElementById('configuracionModal');
                        ...
                        var m = (result.data && result.data.message) ? result.data.message :
