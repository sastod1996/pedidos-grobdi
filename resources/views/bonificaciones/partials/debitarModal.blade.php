    ...
    <div class="modal-dialog modal-dialog-centered modal-md">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="debitarModalBody"
                    data-update-url-template="{{ url('bonificaciones/metas/update-debited-amount/__ID__') }}">
                    <form class="debitar-form" id="formDebitar" action="" method="POST">
                        ...
                        <div class="row g-4">
                            <div class="col-12">
                                <label for="debitarMonto" class="form-label fw-semibold text-dark">Monto
                                    debitado</label>
                                <div class="input-group debitar-input-group">
                                    <span class="input-group-text text-muted">S/</span>
                                    <input type="number" min="0" step="0.01" class="form-control w-full"
                                        id="debitarMonto" name="debited_amount" placeholder="0.00" />
                                </div>
                            </div>
                            <div class="col-12">
                                <label for="debitarObservacion"
                                    class="form-label fw-semibold text-dark">Observación</label>
                                <textarea class="form-control debitar-textarea" id="debitarObservacion" name="debit_comment" rows="4"
                                    placeholder="Agrega los detalles del débito..."></textarea>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        debitarModalEl.addEventListener('show.bs.modal', function(event) {
            ...
            var formatted = now.getFullYear() + '-' + pad(now.getMonth() + 1) + '-' + pad(now
                .getDate()) + ' ' + pad(now.getHours()) + ':' + pad(now.getMinutes()) + ':' + pad(
                now
                .getSeconds());
        ...
        if (formDebitar) {
            formDebitar.addEventListener('submit', function(e) {
                ...
                if (!action) {
                    var container = document.getElementById('debitarModalBody');
                    var template = container ? container.getAttribute('data-update-url-template') :
                        null;
                ...
                if (dtEl && (!dtEl.value || dtEl.value === 'null')) {
                    ...
                    dtEl.value = now2.getFullYear() + '-' + pad2(now2.getMonth() + 1) + '-' + pad2(now2
                            .getDate()) + ' ' + pad2(now2.getHours()) + ':' + pad2(now2.getMinutes()) +
                ...
                if (dtEl) {
                    try {
                        fd.set('debited_datetime', dtEl.value);
                    } catch (e) {
                        fd.append('debited_datetime', dtEl.value);
                    }
                }
                ...
                fetch(action, {
                    ...
                    if (result.ok && result.data && result.data.success) {
                        ...
                        try {
                            ...
                            if (modalEl && window.bootstrap && window.bootstrap.Modal) {
                                var m = window.bootstrap.Modal.getInstance(modalEl) || window
                        ...
                        var row = document.querySelector('tr[data-visitor-goal-id="' +
                            visitorGoalId + '"]');
                        if (row) {
                            ...
                            var debitedDatetime = vg.debited_datetime ?? vg.debitedDatetime ??
                                null;
                            if (typeof debitedAmount !== 'undefined' && tds[7]) {
                                tds[7].textContent = isNaN(parseFloat(debitedAmount)) ? (
                                    debitedAmount ?? '-') : ('S/ ' + parseFloat(
                                    debitedAmount).toLocaleString('es-PE', {
                        ...
                        if (missingDt) {
                            ...
                            var payload = {
                                debited_amount: formDebitar.querySelector(
                                    '[name="debited_amount"]').value,
                                debit_comment: formDebitar.querySelector(
                                    '[name="debit_comment"]').value,
                            ...
                            return fetch(action, {
                                ...
                                if (result2.ok && result2.data && result2.data
                                    .success) {
                                    ...
                                    try {
                                        var modalEl = document.getElementById(
                                            'debitarModal');
                                        if (modalEl && window.bootstrap && window
                                            .bootstrap.Modal) {
                                            var m = window.bootstrap.Modal.getInstance(
                                    ...
                                    var row = document.querySelector(
                                        'tr[data-visitor-goal-id="' +
                                        visitorGoalId + '"]');
                                    if (row) {
                                        var tds = row.querySelectorAll('td');
                                        var debitedAmount = vg.debited_amount ?? vg
                                            .debitedAmount ?? null;
                                        var debitedDatetime = vg.debited_datetime ?? vg
                                            .debitedDatetime ?? null;
                                        if (typeof debitedAmount !== 'undefined' && tds[
                                                7]) {
                                            tds[7].textContent = isNaN(parseFloat(
                                                debitedAmount)) ? (debitedAmount ??
                                                '-') : ('S/ ' + parseFloat(
                                                debitedAmount).toLocaleString(
                                                'es-PE', {
                                                    minimumFractionDigits: 2,
                                                    maximumFractionDigits: 2
                                                }));
                                    ...
                                    if (window.Swal) {
                                        ...
                                        toastr.success(result2.data.message ||
                                            'Monto debitado actualizado.');
                                    } else {
                                    ...
                                    if (result2.data && result2.data.errors) {
                                        for (var k2 in result2.data.errors) {
                                            result2.data.errors[k2].forEach(function(
                                                m) {
                                                messages2.push(m);
                                            });
                                        }
                                ...
                                if (window.Swal) {
                                    Swal.fire({
                                        icon: 'error',
                                        title: 'Error',
                                        text: 'Error al reintentar la solicitud como JSON.'
                                    });
                                } else {
                                    alert(
                        ...
                        if (window.Swal) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Errores de validación',
                                html: '<ul style="text-align:left">' + messages.map(
                                    function(m) {
                                        return '<li>' + m + '</li>';
                                    }).join('') + '</ul>'
                            });
                        ...
                        var msg = (result.data && result.data.message) ? result.data.message :
                            'Ocurrió un error al actualizar el monto debitado.';
                        if (window.Swal) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                    ...
                    if (window.Swal) {
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error de red al enviar el formulario.'
                        });
                    } else {
